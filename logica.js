...
// "medida" -> "Medida". Solo estético, para que quede igual de prolijo
// que la etiqueta que ya se ve en la tarjeta (ver capitalizar() en
// varianteDeMedidas.js y detalleProducto.js).
...
categoriasUnicas.forEach(categoria => {
  ...
  boton.addEventListener("click", () => {
    ...
    varianteDeMedidas.actualizarStockPorVariante()
    varianteDeMedidas.actualizarCascadaDeVariantes()
...
descu.porDeDescuento();
// Refresca el "X disponibles" de las tarjetas con el arreglo nuevo de
// variantes (stock individual por opción), también en el primer pintado
// del catálogo, no solo tras filtrar o buscar.
varianteDeMedidas.actualizarStockPorVariante();
varianteDeMedidas.actualizarCascadaDeVariantes();
...
export function agregarProductoAlCarrito(productId, opciones = {}) {
  ...
  // "let" y no "const": más abajo se completa con valor1 (ej: "Medida")
  // antes de armar la descripción final.
  let { varianteTexto = "" } = opciones;
  ...
  // Productos migrados al arreglo "variantes" tienen stock INDIVIDUAL
  // por opción (ej: 1 unidad en "rojo", 2 en "azul"), en vez de un único
  // stock compartido por todas las opciones. Si es el caso, validamos
  // contra el stock de la opción puntual elegida, no contra el
  // Inventario general del producto.
  const usaVariantesPorArray = buscarDatos.tieneVariantesPorArray(productId);
  const stockAValidar = (usaVariantesPorArray && variante != null && variante !== "")
    ? buscarDatos.buscarStockVariante(productId, variante)
    : stock;

  // Si la variante elegida tiene su propio precio ("precioVenta"), se
  // usa ese en vez del "Venta" general del producto. El DOLAR y el
  // Descuento siguen siendo del producto en general (no son por
  // variante), así que se le siguen aplicando igual más abajo, sin
  // ningún cambio.
  if (usaVariantesPorArray && variante != null && variante !== "") {
    const precioVariante = buscarDatos.buscarPrecioVariante(productId, variante);
    if (precioVariante !== undefined) {
      pre = precioVariante;
    }
  }

  // El "varianteTexto" que llega acá (desde la tarjeta o desde el modal
  // de detalle) sale del <option> elegido en el select de variante, que
  // solo muestra "valor2" (ej: "3"). Le faltaba "valor1" (ej: "medida"),
  // que es el que le da sentido ("Medida 3" en vez de solo "3"). Lo
  // completamos acá, en el único lugar donde se arma la descripción que
  // termina en el carrito, para que funcione igual sin importar si se
  // agregó desde la tarjeta o desde el modal.
  if (usaVariantesPorArray && variante != null && variante !== "") {
    const valor1Variante = buscarDatos.buscarValor1Variante(productId, variante);
    if (valor1Variante) {
      varianteTexto = `${capitalizar(valor1Variante)} ${varianteTexto}`.trim();
    }
  }

  // Sumamos las unidades ya en el carrito que compiten por ESE MISMO
  // stock antes de validar:
  //  - Sistema viejo: TODAS las variantes del mismo producto, porque
  //    comparten un único Inventario (si hay 10 en stock y ya tenés 8
  //    de una variante, no podés agregar 8 más de otra: serían 16 de un
  //    producto con solo 10 disponibles).
  //  - Sistema nuevo: solo las unidades de la MISMA opción (idVariante),
  //    porque cada una tiene su propio stock independiente.
  const unidadesYaEnCarrito = itemCarrito.reduce((totalAcumulado, item) => {
    ...
    if (idBaseItem !== productId) return totalAcumulado;
    if (usaVariantesPorArray) {
      return String(item.IdVariante) === String(variante) ? totalAcumulado + item.Unidades : totalAcumulado;
    }
    return totalAcumulado + item.Unidades;
  }, 0);

  if (stockAValidar !== undefined && unidadesYaEnCarrito + unidades > stockAValidar) {
  ...
  let agregarOModificarItem = (articuloId, Artículo, Descripción, Venta, DOLAR, Unidades, Descuento, ImagenId, IdVariante) => {
    ...
    if (siEstaId) {
      ...
      if (Descuento != 0) {
        ...
        // corresponde a ningún archivo de imagen real). "IdVariante" se
        // guarda para el sistema nuevo, para poder validar el stock de
        // ESA opción puntual más adelante (ej: al tocar "+" en el carrito).
        itemCarrito.push({ Artículo, Descripción, Venta: ventaCD.toString(), DOLAR, Unidades, ImagenId, IdVariante });
        localStor.guardarEnLocalStorage(itemCarrito);
        agregar(Descripción, articuloId);
      } else {
        itemCarrito.push({ Artículo, Descripción, Venta, DOLAR, Unidades, ImagenId, IdVariante });
        localStor.guardarEnLocalStorage(itemCarrito);
        agregar(Descripción, articuloId);
      }
    }
  ...
  if (medida == null && variante == null) {
    agregarOModificarItem(productId, productId, tit, pre, dol, unidades, desc, productId, undefined);
  } else {
    // "medidaSegura"/"medidaTextoFinal": en el sistema nuevo no hay
    // select de "medida" (solo el de variante), así que "medida" llega
    // null. Antes esto pegaba el texto literal "null" en el nombre del
    // producto y en el id compuesto del carrito (null + '9990' + ... =
    // "null9990..."); lo evitamos con un string vacío. Para el sistema
    // viejo esto no cambia nada (ahí "medida" siempre tiene un valor real).
    let medidaSegura = medida == null ? '' : medida;
    let medidaTextoFinal = medida == null ? '' : medidaTexto;
    let varianteTextoFinal = variante == null ? "" : varianteTexto;
    let articuloIdModificado = medidaSegura + '9990' + productId + variante;
    let descripcionFinal = `${tit} ${medidaTextoFinal} ${varianteTextoFinal}`.replace(/\s+/g, ' ').trim();
    agregarOModificarItem(articuloIdModificado, (parseInt(articuloIdModificado)), descripcionFinal, pre, dol, unidades, desc, productId, variante);
  }
...
function cambiarCantidadCarrito(articuloId, delta) {
  ...
  if (delta > 0) {
    ...
    // solo compite contra las unidades de esa MISMA opción (item.IdVariante),
    // no contra las de otras opciones del mismo producto. Los ítems
    // guardados en carritos de antes de este cambio no tienen
    // "IdVariante", así que quedan tratados como sistema viejo (respaldo).
    const usaVariantesPorArray = buscarDatos.tieneVariantesPorArray(idRealProducto) && item.IdVariante !== undefined;

    // Sistema viejo: el stock es del producto en general, así que
    // sumamos las unidades de TODAS las variantes de este mismo producto
    // que ya estén en el carrito (menos este item, que ya se cuenta en
    // "nuevasUnidades").
    const unidadesDeOtrasVariantes = itemCarrito.reduce((total, otro) => {
      ...
      if (idBaseOtro !== idRealProducto) return total;
      if (usaVariantesPorArray) {
        return String(otro.IdVariante) === String(item.IdVariante) ? total + otro.Unidades : total;
      }
      return total + otro.Unidades;
    }, 0);

    const stock = usaVariantesPorArray
      ? buscarDatos.buscarStockVariante(idRealProducto, item.IdVariante)
      : buscarDatos.buscarStock(idRealProducto);
